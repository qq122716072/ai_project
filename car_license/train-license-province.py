...
SAVER_DIR = "train-saver/province/me"
...
if __name__ == '__main__' and sys.argv[1] == 'train':
    ...
    with tf.Session() as sess:
        ...
        corrcet_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
        ...
        # 譬如,如果每次训练op的个数是60,图片总数为150张,则前面两次各输入60张,最后一次输入30张(余数30)
        batch_size = 60
        iterations = iterations
        batches_count = int(input_count / batch_size)
        remainder = input_count % batch_size
        print('训练数据分成 %s 批,前面每批 %s 个数据,最后一批 %s 个数据' % (batches_count + 1, batch_size, remainder))

        # 执行训练迭代
        for it in range(iterations):
            # 这里的关键是要把输入数据转化为np.array
            ...
            iterate_accuracy = 0
            if it % 5 == 0:
                iterate_accuracy = accuracy.eval(feed_dict={x: val_images, y_: input_labels,keep_prob: 1.0})
