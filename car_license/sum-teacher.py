        ...
        if (white[n] if arg else black[n]) > (0.05 * white_max if arg else 0.05 * black_max):
            ...
            if end - start > 5:  # 车牌左边白条移除
                ...
                if temp == 1 and end - start < 20:
                    ...
                    shutil.copy(
                        os.path.join("./tf_car_license_dataset/train_images/training-set/1/", "111.bmp"),  # 111.bmp 是一张 1 的样本图片
                        os.path.join("./img_cut/", str(temp) + '.bmp'))
                    pass
                else:
                    cj = th3[1:height, start:end]
                    dir = './img_cut_not_3240/'
    ...
    with province_graph.as_default():
        with tf.Session(graph=province_graph) as sess_p:
            ...
            saver_p = tf.train.import_meta_graph(
                "./train-saver/province/teacher/model.ckpt.meta")
    ...
    with letter_graph.as_default():
        with tf.Session(graph=letter_graph) as sess:
            ...
            saver = tf.train.import_meta_graph(
                "./train-saver/letters/teacher/model.ckpt.meta")
            model_file = tf.train.latest_checkpoint("./train-saver/letters/teacher/")
...
def last_5_num_test():
    ...
    with last_5_num_graph.as_default():
        with tf.Session(graph=last_5_num_graph) as sess:
            ...
            x_image = tf.reshape(x, [-1, WIDTH, HEIGHT, 1])
            saver = tf.train.import_meta_graph(
                "./train-saver/digits/teacher/model.ckpt.meta")
            model_file = tf.train.latest_checkpoint("./train-saver/digits/teacher")
            print("main3")
            saver.restore(sess, model_file)
